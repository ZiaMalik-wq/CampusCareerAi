...
import { Search, Sparkles, Database, Layers, X, Filter, SlidersHorizontal, TrendingUp } from 'lucide-react';
...
const Jobs = () => {
  ...
  // New: Filter states (add these when you have filters on backend)
  ...
  // Get mode info for better UX - FIXED with explicit classes
  const getModeInfo = (mode) => {
    const modes = {
      hybrid: {
        ...
        badge: 'Recommended',
        activeClasses: 'bg-purple-600 border-purple-600 text-white shadow-lg shadow-purple-500/30',
      ...
      semantic: {
        icon: Sparkles,
        label: 'AI Semantic',
        description: 'Natural language search that understands context and meaning',
        badge: 'Smart',
        activeClasses: 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-500/30',
        bgClass: 'bg-blue-50',
      ...
      sql: {
        ...
        badge: 'Classic',
        activeClasses: 'bg-green-600 border-green-600 text-white shadow-lg shadow-green-500/30',
        bgClass: 'bg-green-50',
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">Powered by intelligent search • {jobs.length} positions available</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-gray-100 p-6 md:p-8 mb-8">
          ...
          <form onSubmit={handleSearchSubmit} className="space-y-4">
            ...
            {/* Main Search Bar with Mode Selector Combined */}
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              {/* Search Mode Selector - Styled as Pills with FIXED classes */}
            ...
            {/* Active Mode Description - FIXED with explicit classes */}
            <div className={`flex items-start gap-3 p-4 rounded-2xl ${currentMode.bgClass} border ${currentMode.borderClass}`}>
              <div className={`p-2 ${currentMode.iconBgClass} rounded-lg mt-0.5`}>
                <ModeIcon className={`w-4 h-4 ${currentMode.iconColorClass}`} />
              </div>
              <div className="flex-grow">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`font-semibold ${currentMode.textColorClass}`}>
                    {currentMode.label}
                  </span>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${currentMode.badgeBgClass} ${currentMode.badgeTextClass} font-medium`}>
                    {currentMode.badge}
                  </span>
                </div>
          ...
          {!searchQuery && (
            <div className="mt-6 pt-6 border-t border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                {['Remote React Developer', 'Data Analyst NYC', 'Marketing Intern', 'Full Stack Engineer'].map((suggestion) => (
                  <button
                    key={suggestion}
                    type="button"
                    onClick={() => {
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-20">
            ...
            <p className="text-gray-500 mt-6 animate-pulse font-medium">
              {searchMode === 'semantic' ? 'AI is analyzing your query...' : 'Searching opportunities...'}
            </p>
          </div>
        ) : error ? (
