...
const InterviewPrep = () => {
  ...
  useEffect(() => {
    ...
    const generatePrep = async () => {
      try {
        ...
        const msg = err.response?.data?.detail || "Failed to generate interview prep.";
        setError(msg);
        toast.error(msg);
      } finally {
        setLoading(false);
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex flex-col justify-center items-center p-4">
      ...
      <h2 className="mt-8 text-2xl font-bold text-gray-800">AI is analyzing your resume...</h2>
      <p className="text-gray-500 mt-2 text-center max-w-md">
        We are comparing your skills against the job description to find your gaps and generate custom questions.
      </p>
  ...
  if (error) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="text-center max-w-md">
        ...
        <button onClick={() => navigate(`/jobs/${id}`)} className="px-6 py-2 bg-white border border-gray-300 rounded-lg font-semibold hover:bg-gray-50">
          Back to Job
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Interview Prep Plan</h1>
            <p className="text-gray-600">AI-generated questions based on your skill gaps.</p>
          </div>
        </div>
        ...
        <div className="mb-10">
          ...
          <div className="space-y-4">
            {prepData?.technical_questions.map((q, idx) => (
              <div key={idx} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span className={`text-xs px-2.5 py-1 rounded-full font-bold border ${getDifficultyColor(q.difficulty)}`}>
                      {q.difficulty}
                    </span>
                    {expandedIndex === idx ? (
                  ...
                  <div className="px-5 pb-5 pt-0 animate-in slide-in-from-top-2 duration-200">
                    <div className="mt-3 p-4 bg-purple-50 rounded-lg border border-purple-100">
                      ...
                      <ul className="space-y-2">
                        ...
                        {q.expected_answer_key_points.split(';').map((point, i) => (
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2">
            <User className="w-6 h-6 text-blue-600" /> {/* Reusing User Icon logic or import it */}
            Behavioral Questions
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {prepData?.behavioral_questions.map((q, idx) => (
              <div key={idx} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col h-full">
