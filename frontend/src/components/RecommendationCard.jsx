...
const RecommendationCard = ({ job }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition duration-300 relative overflow-hidden">
      ...
      <div className={`absolute top-0 right-0 p-4 rounded-bl-2xl border-b border-l ${scoreClass}`}>
      ...
      <div className="pr-16"> {/* Padding right to avoid overlap with badge */}
        ...
        {job.matching_skills.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {job.matching_skills.map((skill, index) => (
              <span key={index} className="inline-flex items-center gap-1 px-2 py-1 bg-green-100 text-green-700 text-xs rounded-md font-medium">
        ...
        {job.missing_skills.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {job.missing_skills.slice(0, 3).map((skill, index) => (
              <span key={index} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 text-gray-500 text-xs rounded-md border border-gray-200">
